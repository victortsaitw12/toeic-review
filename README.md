...
純靜態網站。導覽分為：**今天**（儀表板）、**英文聽力**（真題聽力／聽力解析／口說跟讀／聽寫練習）、
**英文單字**（單字卡／單字測驗）、**英文填空**、**模擬考試**、**單字管理**。包含：
...
- **聽力解析** — 5 份模擬考聽力，已切成每題（Part 1/2）或每組對話（Part 3/4）一個音檔，
  可逐題選擇播放，支援變速（0.6×–1.15×）、單檔循環、A-B 區間循環、倒退 3 秒。
- **真題聽力（逐題快答）**（模擬考試分頁）— **EZ 模擬考 5 回＋國學聽力藍本 10 回**的
  真題音檔逐題練，可選系列/回數／Part／題數：Part 1 看圖片聽四個描述選字母
  （已收錄題本圖片的回數直接顯示照片，其餘看題本；見下方「題本圖片」）；
  ...
  Part 2 題庫（`data/listen2.js`，27 回共 675 題）由 `tools/extract_part2.py` 從逐字稿
  切出題目與三個回應（正確答案由 Claude 判定，存 `review/part2_ans/*.tsv`，`--merge` 併入；
  EZ 125 題與藍本 250 題已對過官方答案卷，僅 1 題因轉錄聽錯修正）——**國學 A/B 本 12 回
  沒有官方答案卷，在真題聽力中以「僅 Part 2」形式提供**（原獨立的「真人聽力 Part 2 快答」
  已於 2026-07-19 併入真題聽力）。
...
- **TOEIC 核心搭配詞** — `tools/extract_colloc.py` 維護 117 條高頻商用搭配詞
  ...
  翻卡／測驗／間隔複習全部沿用單字卡機制。
- **英文填空**（原「錯題複習」）— 做模擬考（尤其 Part 5）記錄下來的錯題／不熟題，可反覆練習並追蹤答對答錯。
...
### 題本圖片（Part 1 照片、Part 3/4 圖表題）

翻拍題本的圖片後可讓逐題快答直接顯示，不用翻書：

1. 圖片放 `img/<Key>/`，命名 `q<題號>.png`（Part 1 單題）或 `q<起>-<迄>.png`（圖表題組共用一張）；
   掃描歪掉的先用 `python3 tools/deskew.py 輸入.png 輸出.png` 自動轉正
   （偵測不到角度的照舊輸出，轉完務必看圖確認）
2. 在 `data/examimg.js` 的 `EXAM_IMG["<Key>"]` 登錄「題號 → 檔名」
   （題組的三個題號都要指向同一檔）
3. 前端自動生效：真題聽力（逐題快答）有登錄的題目直接顯示圖片，沒登錄的照舊顯示
   「請對照題本」；**聽力解析／口說跟讀／聽寫練習**選到對應音檔（由檔名
   `PartN_Qxx` 反查）也會在播放器下方顯示同一張圖

> Part 3/4 圖表題（表格、地圖、圖表、平面圖）原掃描解析度低、放大後文字糊，
> 已依內容用 `tools/redraw_graphics.py` **重繪為向量 SVG**（任意縮放不失真），
> 登錄檔名改為 `q<起>-<迄>.svg`。Part 1 照片維持原掃描 PNG。

目前已收錄：
- **Test1（EZ 第 1 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100
- **Test2（EZ 第 2 回）**——Part 1 全 6 題＋圖表題組 65–67、68–70、92–94、95–97、98–100
- **Test3（EZ 第 3 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100
- **Test4（EZ 第 4 回）**——Part 1 全 6 題＋圖表題組 65–67、68–70、92–94、95–97、98–100
- **Test5（EZ 第 5 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100
- **TestC1（國學藍本 第 1 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100
- **TestC2（國學藍本 第 2 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100
- **TestC3（國學藍本 第 3 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG）
- **TestC4（國學藍本 第 4 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG）
- **TestC5（國學藍本 第 5 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG；
  圖片與音檔為同版本，Q63/66/69/99 的選項已依圖改成題本真正選項）
- **TestC6（國學藍本 第 6 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG；
  圖片與音檔為同版本，Q32–100 全數依題本掃描圖訂正，圖表題 63/67/69/96/98 用題本真正選項）
- **TestC7（國學藍本 第 7 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG；
  圖片與音檔為同版本，Q32–100 全數依題本掃描圖訂正，圖表題 64/66/68/95/100 用題本真正選項；
  Q66 座位圖的已售灰底掃描時被壓成純白，改依錄音＋答案卷反推重建，解析有註明）
- **TestC8（國學藍本 第 8 回）**——Part 1 全 6 題＋圖表題組 62–64、65–67、68–70、95–97、98–100（圖表題重繪 SVG；
  圖片與音檔為同版本，Q32–100 全數依題本掃描圖訂正，圖表題 63/66/69/96/99 用題本真正選項）
