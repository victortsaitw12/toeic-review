...
國學 A/B 本的「04 單字MP3」是**英文、中文交錯朗讀**，而 whisper 轉錄的 TSV
（各教材資料夾的 `04_單字MP3_轉錄/`）英文、中文各自獨立一行，英文那行的起訖秒數
就是單字邊界，所以不需要重新做靜音切割，直接依時間戳切段即可：
...
- whisper 偶爾會把整個檔切成**等長格點**（例如每段都剛好 2.0 秒），那種時間戳不是
  ...
  （目前 96 個檔中有 8 個），這些字就退回 TTS。
- 重跑會依 `audio/word/.manifest.json` 只重切來源或時間戳有變的字，並清掉過期片段。
- 綠本（`國學多益寫作綠本單字/轉錄/`）的英中混在同一行、時間戳也不可靠，
  目前**沒有**納入；要納入需先用 `word_timestamps=True` 重跑轉錄拿字級邊界。
