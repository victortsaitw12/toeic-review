...
tests = {k: v for k, v in tests.items() if not re.match(r'Test[AB]\d', k)}

gx = json.load(open('/workspaces/toeic/gx_tests.json'))
for key in sorted(gx):
...
for book, zh in (('A', 'A本'), ('B', 'B本')):
    for t in range(1, 7):
        comma = '' if (book == 'B' and t == 6) else ','
        meta_lines.append(f'  "Test{book}{t}": {{"name": "國學{book} 模擬考 {t}", "source": "國學多益模擬題{zh}"}}{comma}')
meta_lines.append('};')
